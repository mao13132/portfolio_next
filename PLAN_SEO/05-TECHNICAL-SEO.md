...
> **⚠️ НЕ ДЕЛАТЬ НА ФРОНТЕНДЕ! robots.txt обслуживается бекендом (FastAPI).**
> Не создавать файл в `public/robots.txt` — он раздаётся бекендом.

### Текущее состояние: ✅ На бекенде (FastAPI) — НЕ НУЖНО на фронтенде

### ~~Что нужно сделать:~~ (НЕ АКТУАЛЬНО ДЛЯ ФРОНТЕНДА)
...
> ✅ **Решено на бекенде (FastAPI).** На фронтенде ничего делать не нужно.

---

## 2. Sitemap.xml

> **⚠️ НЕ ДЕЛАТЬ НА ФРОНТЕНДЕ! Sitemap.xml обслуживается бекендом (FastAPI).**
> Не создавать `pages/sitemap.xml.tsx` и не устанавливать `next-sitemap`.
